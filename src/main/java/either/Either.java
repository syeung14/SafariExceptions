...
public class Either<S, F> {
    ...
    public S get() {
        if (isSuccess()) {
            ...
            //runtime exception, not check to be used with lambda
    ...
    public static <S, F> UnaryOperator<Either<S, F>> retries(UnaryOperator<Either<S, F>> op, int limit) {
        return e -> {
            int triesLeft = limit;
    ...
    public static <S, F> UnaryOperator<Either<S, F>> retrySequence(UnaryOperator<Either<S, F>>... op) {
        return e -> {
            int idx = 0;
...
class UseEither {
    ...
    private static Either<Stream<String>, Throwable> tryBackup(Either<Stream<String>, Throwable> e) {
    ...
    private static UnaryOperator<Either<Stream<String>, Throwable>> pause(int delay) {
        return e -> {
            try {
                System.out.println(Colors.PURPLE + "pausing for: " + delay + " millis" + Colors.RESET);
                Thread.sleep(delay);
